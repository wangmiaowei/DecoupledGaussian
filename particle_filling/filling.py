...
def fill_particles(
    ...
    particles_index = particles_tensor.shape[0]
    if concat == True:
        particles_index = [pos_clone.shape[0], pos_clone.shape[0] + particles_tensor.shape[0]]
        particles_tensor = torch.cat([pos_clone, particles_tensor], dim=0)
    
    return (particles_tensor, particles_index)
