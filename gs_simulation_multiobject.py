...
def load_multi_objects_checkpoint(scene_path, object_path, sh_degree=3, z_offset=0):
    gaussians = GaussianModel(sh_degree)
    gaussians.load_multi_objects_ply(scene_path=scene_path, object_path=object_path, z_offset=z_offset)
    return gaussians
...
if __name__ == "__main__":
    ...
    gaussians = load_multi_objects_checkpoint(scene_path, object_path, z_offset=z_offset)
    ...
    mpm_init_pos = transformed_pos
    
    if filling_params is not None:

        for i in range(gaussians._onum):

            print(f"??{gaussians._object_indexs[i]}_{i}")

            obj_init_pos = transformed_pos[gaussians._object_indexs[i][0]: gaussians._object_indexs[i][1]].detach().cpu().numpy()
            ...
            particles_tensor_, particles_index_ = fill_particles(
                pos=transformed_pos,
            ...
            mpm_init_pos = torch.cat([mpm_init_pos, particles_tensor_], dim=0) 
            
            print(f"particles_index_{i}: {particles_index_}")
            if particles_index is None:
                particles_index = [transformed_pos.shape[0], transformed_pos.shape[0] + particles_index_]
            else: 
                particles_index = [particles_index[0], particles_index[1] + particles_index_]
    ...
    # print(mpm_solver.mpm_state.particle_selection)
