...
def decode_param_json(json_file):
    ...
    z_offset = 0
    if "z_offset" in sim_params.keys():
        z_offset = sim_params["z_offset"]
    ...
    return material_params, bc_params, time_params, preprocessing_params, camera_params, z_offset


def set_boundary_conditions(
    mpm_solver: MPM_Simulator_WARP, bc_params: dict, time_params: dict, gaussians: None
):
    for bc in bc_params:
        if bc["type"] == "cuboid":
            assert (
            ...
            bbox = gaussians._object_bbox
            mpm_solver.tear_object(
                force=bc["force"],
        ...
        elif bc["type"] == "enforce_scene_translation":
            assert "velocity" in bc.keys()
            assert "start_time" in bc.keys()
            assert "end_time" in bc.keys()
