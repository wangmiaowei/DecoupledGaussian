...
import torch
from internal.utils.camera import readColmapSceneInfo,cameraList_from_camInfos
...
class Viewer:
    def __init__(
        ...
        if seganygs is None and load_from.endswith(".ckpt"):
            ...
            if os.path.exists(seganygs_tag_file_path) is True:
                ...
                seganygs = load_from
                #print('segangs_tag_fiel_path: ',seganygs_tag_file_path)
        ...
        if vanilla_seganygs is True:
            ...
            load_from = os.path.join(load_from, "point_cloud.ply")
        ...
        # load camera poses
        print('cameras_json_path: ',cameras_json_path)
        self.camera_poses = self.load_camera_poses(cameras_json_path)
        self.obj_name = cameras_json_path.split("/")[-2]
        try:
            scene_info=readColmapSceneInfo("../input_dataset/"+self.obj_name,"images",False)
            self.trainning_views = cameraList_from_camInfos(scene_info.train_cameras, resolution_scale=1.0)
        except:
            self.trainning_views = None
        #self.trainning_views = self.trainnig_views_convert(self.camera_poses)
        # calculate camera center
        if len(self.camera_poses) > 0:
            positions = np.asarray([i["position"] for i in self.camera_poses])
            self.camera_center = np.mean(positions, axis=0)
            # 计算最小和最大坐标
            min_pos = np.min(positions, axis=0)
            max_pos = np.max(positions, axis=0)
            # 计算最大长宽高
            self.camera_dimensions = max_pos - min_pos
    ...
    def calculate_ndc_projection_matrix(self,fov_y, fov_x, world_to_camera):
        """
        Calculate the NDC projection matrix.
        
        Args:
            fov_y (torch.Tensor): Vertical field of view (in radians).
            fov_x (torch.Tensor): Horizontal field of view (in radians).
            world_to_camera (torch.Tensor): 4x4 transformation matrix from world to camera coordinates.

        Returns:
            torch.Tensor: The full NDC projection matrix (4x4).
        """
        zfar = 100.0
        znear = 0.01
        ...
        # Initialize projection matrix as a 4x4 tensor
        ...
        full_projection = torch.matmul(world_to_camera, P.T.to(world_to_camera.dtype))  # Result is (4, 4)

        return full_projection
    def trainnig_views_convert(self,camera_poses):
        trainning_views=[]
        """
        ...
        for camera_pose in camera_poses:
            training_view = {}
            training_view["img_name"] = camera_pose["img_name"]
            training_view["image_width"] = torch.tensor(camera_pose["width"]).to(torch.float32).cuda()
            training_view["image_height"] = torch.tensor(camera_pose["height"]).to(torch.float32).cuda()
            c2w = np.eye(4)
            c2w[:3, :3] = np.asarray(camera_pose["rotation"])
            c2w[:3, 3] = np.asarray(camera_pose["position"])
            # Set up rasterization configuration
            
            fov_x = 2 * torch.atan((training_view["image_width"] / 2) / torch.tensor(camera_pose["fx"])).to(torch.float32).cuda()
            fov_y = 2 * torch.atan((training_view["image_height"] / 2) / torch.tensor(camera_pose["fy"])).to(torch.float32).cuda()
            
            tanfovx = torch.tan(fov_x * 0.5).to(torch.float32).cuda()
            tanfovy = torch.tan(fov_y * 0.5).to(torch.float32).cuda()
            training_view["R"] =  np.asarray(camera_pose["rotation"])
            training_view["T"] =  np.asarray(camera_pose["position"])
            print('trainning_view[R]: ',training_view["R"])
            print('training_view["T"]: ',training_view["T"])
            break
            training_view["fx"] =  np.asarray(camera_pose["fx"])
            training_view["fy"] =  np.asarray(camera_pose["fy"])

            training_view["cx"] =  0.5*training_view["image_width"]
            training_view["cy"] =  0.5*training_view["image_height"]

            training_view["tanfovx"] =  tanfovx.cuda()
            training_view["tanfovy"] =  tanfovy.cuda()
            training_view["viewmatrix"] = torch.tensor(np.linalg.inv(c2w)).to(torch.float32).cuda().T # world to camera matrix
            training_view["camera_center"] = torch.linalg.inv(training_view["viewmatrix"])[3, :3].cuda()
            training_view["full_projection"] = self.calculate_ndc_projection_matrix(fov_y, 
                                                                                    fov_x,training_view["viewmatrix"]).to(torch.float32).cuda()
            trainning_views.append(training_view)
        return trainning_views
    @staticmethod
    def _search_load_file(model_path: str) -> str:
        model_path = model_path.replace("/home/wmw/project/exp_res","/mnt/wmw/exp_res")
    ...
    def _load_seganygs(self, path):
        ...
        semantic_features = (model.get_processed_semantic_features()).cuda()
        scale_gate = model.scale_gate
        del model
        del ckpt
        torch.cuda.empty_cache()
        #semantic_features = semantic_features[self.gaussian_model.mask,:]
