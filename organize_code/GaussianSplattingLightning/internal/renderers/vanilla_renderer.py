...
class VanillaRenderer(Renderer):
    ...
    def forward(
        ...
        """
        viewmatrix = torch.tensor([[-0.9882, -0.1514,  0.0222,  0.0000],
                                   [-0.1097,  0.8021,  0.5870,  0.0000],
                                   [-0.1066,  0.5777, -0.8093,  0.0000],
                                   [ 0.8892, -1.5053,  3.3305,  1.0000]], device='cuda:0')
        projmatrix = torch.tensor([[-1.2879, -0.2963,  0.0222,  0.0222],
                                   [-0.1429,  1.5704,  0.5871,  0.5870],
                                   [-0.1390,  1.1311, -0.8093, -0.8093],
                                   [ 1.1589, -2.9473,  3.3208,  3.3305]], device='cuda:0')
        """
        ...
        #up vector = tensor([ 0.0642, -0.7863, -0.6145])
        #up vector = tensor([ 0.1315, -0.9654, -0.2254])
        #cameras_json_path:  /mnt/wmw/exp_res/figurines/gsplat/cameras.json
        #print('campos: ',viewpoint_camera.camera_center)
        #campos:  tensor([ 3.9216, -2.9660,  1.6805], device='cuda:0')

        rasterizer = GaussianRasterizer(raster_settings=raster_settings)

        means3D = pc.get_xyz
        means2D = screenspace_points
        #opacity = torch.ones(pc.get_opacity.shape).cuda()
