...
class SegAnySplatting(lightning.LightningModule):
    def __init__(
        ...
        self.rfn = rfn
    def loss_cls_3d(self, k=5, lambda_val=2.0, max_points=600000, sample_size=4000):
        """
        ...
        """
        predictions = self.gaussian_semantic_features
        features = self.models.gaussian.get_xyz.squeeze().detach()
        ...
        # 扩展A的维度
        A_expanded = sample_preds.unsqueeze(1)  # 变为[800, 1, 32]
        # 计算点积
        dot_product = torch.sum(A_expanded * neighbor_preds, dim=2)  # 变为[800, 5]
        # 计算A和B的范数
        A_norm = torch.norm(A_expanded, dim=2)  # 变为[800, 1]
        B_norm = torch.norm(neighbor_preds, dim=2)  # 变为[800, 5]
        # 计算余弦相似度
        cosine_similarity = dot_product / (A_norm * B_norm + 1e-8)  # 添加一个小值以避免除以零
        target_similarity = 1.0
        loss = torch.mean((cosine_similarity - target_similarity) ** 2)
    ...
    def forward_with_loss_calculation(self, camera, image_info, semantic):
        ...
        loss = (- per_pixel_weight[:, sampled_mask_positive] * gt_corrs[:, sampled_mask_positive] * corr[:, sampled_mask_positive]).mean() \
               ...
               #+ self.rfn * rendered_feature_norm_reg+kl_loss
        with torch.no_grad():
