...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
        ...
        if self.preload_img:
            try:
                frame_name = self.image_path.split("/")[-1]
                mask_numpy_path =frame_name.split(".")[0] +".npy"
                mask_path = os.path.join(Path(self.image_path).parent.parent, "sam2_mask", mask_numpy_path)
                mask = np.load(mask_path)
                image = Image.open(self.image_path)
                resized_mask_rgb = torch.tensor(mask).unsqueeze(0)
                self.mask_image = resized_mask_rgb.to(self.data_device)
                self.mask_flag = True
            except:
                try:
                    image = Image.open(self.image_path.replace(".jpg",".png"))
                except:
                    image = Image.open(self.image_path.replace(".JPG",".jpg"))
    ...
    def rotation_matrix_torch(self,roll, pitch, yaw):
        ...
        # The final rotation matrix R = Rz * Ry * Rx
        R = torch.mm(R_z, torch.mm(R_y, R_x))
        return R
    
    def random_pose(self,center,distance):
        yaw = torch.tensor(random.uniform(0, 2 * torch.pi))  # random yaw angle
        pitch = torch.tensor(random.uniform(0, torch.pi))  # random pitch angle
        roll = torch.tensor(0.0)  # you can change this as needed

        # Calculate the position of the camera (eye)
        eye = center + distance * torch.tensor([torch.sin(pitch) * torch.cos(yaw), 
                                                torch.sin(pitch) * torch.sin(yaw), 
                                                torch.cos(pitch)], dtype=torch.float32).cuda()


        R = self.rotation_matrix_torch(roll, pitch, yaw)

        # Construct the 4x4 pose matrix of the camera
        camera_pose = torch.eye(4, dtype=torch.float32).cuda()  # Create the identity matrix
        camera_pose[:3, :3] = R  # Place the rotation matrix in the top left corner of the pose matrix
        camera_pose[:3, 3] = eye  # Place the camera position in the last column of the pose matrix

        
        matrix = torch.linalg.inv(camera_pose)
        R = -torch.transpose(matrix[:3,:3], 0, 1)
        T = -matrix[:3, 3]

        world_view_transform = torch.tensor(getWorld2View2(R.detach().cpu().numpy(), T.detach().cpu().numpy(), 
                                                           self.trans, self.scale)).transpose(0, 1).cuda()
    ...
    def get_image(self):
        if self.preload_img:
            if self.mask_flag:
                return self.original_image.cuda(),self.mask_image.cuda(), self.image_gray.cuda()
            else:
