...
class GaussianModel:
    ...
    def update_learning_rate(self, iteration):
        ...
        if iteration%100==0:
            ...
            mask = (self.opacity_activation(self._opacity)>0.01)[:,0].clone().detach()
            print('number of object Gaussians after UNCE: ',len(mask))
    ...
    def save_ply(self, path, mask=None):
        ...
        obj_xyz = xyz[self.length_scene:]
        normals = np.zeros_like(xyz)[self.length_scene:]
        f_dc = (self._features_dc[self.length_scene:]).detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()

        f_rest = (self._features_rest[self.length_scene:]).detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        opacities = (self._opacity[self.length_scene:]).detach().cpu().numpy()
        scale = (self._scaling[self.length_scene:]).detach().cpu().numpy()
        rotation = (self._rotation[self.length_scene:]).detach().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]
        elements = np.empty(obj_xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((obj_xyz, normals, f_dc, f_rest, opacities, scale, rotation), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)

    def reset_opacity(self):
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity)*0.04))
    ...
    def load_ply(self, obj_repair_path):
        xyz2,features_dc2,features_extra2,opacities2,scales2,rots2 = self.load_single_ply(obj_repair_path)
        
    
        dist = torch.sqrt(torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(xyz2)).float().cuda()), 0.0000001))
        features_extra2 = torch.zeros(features_extra2.shape)
        scales2 = torch.log(dist)[...,None].repeat(1, 3)
        rots2 = torch.zeros((xyz2.shape[0], 4), device="cuda")
        rots2[:, 0] = 1

        opacities2 = inverse_sigmoid(0.15 * torch.ones((xyz2.shape[0], 1), dtype=torch.float, device="cuda"))
        
        self._scaling = nn.Parameter(scales2.cuda().requires_grad_(True)) #

        self._xyz = nn.Parameter(xyz2.cuda().requires_grad_(True))

        #features_dc2 = torch.ones(features_dc2.shape) * features_dc2.mean(dim=0)

        self._features_dc = nn.Parameter(features_dc2.cuda().requires_grad_(True))

        self._features_rest = nn.Parameter(features_extra2.cuda().requires_grad_(True))

        self._opacity = nn.Parameter(opacities2.cuda().requires_grad_(True))

        self._rotation = nn.Parameter(rots2.cuda().requires_grad_(True))
        
        self.active_sh_degree = self.max_sh_degree
        knn_f = torch.randn((xyz2.shape[0], 6)).float().cuda()
        self._knn_f = nn.Parameter(knn_f.requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self.max_weight = torch.zeros((self.get_xyz.shape[0]), device="cuda")
