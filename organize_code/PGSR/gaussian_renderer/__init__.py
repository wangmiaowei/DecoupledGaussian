...
def render_random(viewpoint_camera,random_world_view_transform,random_full_proj_transform,random_camera_center, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
           app_model=None, return_plane = True, return_depth_normal = True):
    """
    Render the scene. 
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            ...
            dir_pp = (pc.get_xyz - random_camera_center.repeat(pc.get_features.shape[0], 1))
    ...
    raster_settings = PlaneGaussianRasterizationSettings(
            ...
            viewmatrix=random_world_view_transform,
            projmatrix=random_full_proj_transform,
            sh_degree=pc.active_sh_degree,
            campos=random_camera_center,
            prefiltered=False,
    ...
    #print('return_plane: ',return_plane)
    if not return_plane:
    ...
    #,random_world_view_transform,random_full_proj_transform,random_camera_center
    local_normal = global_normal @ random_world_view_transform[:3,:3]
    pts_in_cam = means3D @ random_world_view_transform[:3,:3] + random_world_view_transform[3,:3]
...
def render(viewpoint_camera, pc, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, 
           app_model = None, return_plane = True, return_depth_normal = True,correct_norml=False):
    """
    Render the scene. 
    ...
    rendered_image, radii, out_observe, out_all_map, plane_depth = rasterizer(
        ...
        cov3D_precomp = cov3D_precomp)
    if correct_norml:
        pc.get_correct_normal(global_normal,(out_observe>0) & (radii>0))
    rendered_normal = out_all_map[0:3]
    rendered_alpha = out_all_map[3:4, ]
    rendered_distance = out_all_map[4:5, ]
