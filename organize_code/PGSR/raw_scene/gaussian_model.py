...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int):
        ...
        #trying to reestimate the normals
        self._normals = None
        self._normals_mask = torch.empty(0)        
    ...
    def get_correct_normal(self,normals,mask):
        croppped_initial_normals = (self.get_smallest_axis())[mask]
        cropped_normals = normals[mask]
        # 计算两个向量的点积来确定是否同向
        dot_product = torch.sum(croppped_initial_normals * cropped_normals, dim=1)
        # 判断是否同向：点积大于0表示同向，小于或等于0表示方向不同
        same_direction = dot_product > 0
        self._normals_mask[mask] += (same_direction.float() * 2 - 1)
        # minus -0.5,0.5
        other_croppped_initial_normals = (self.get_smallest_axis())[~mask]
        other_cropped_normals = normals[~mask]
        other_dot_product = torch.sum(other_croppped_initial_normals  * other_cropped_normals, dim=1)
        other_same_direction = other_dot_product < 0
        self._normals_mask[~mask] += 0.0001*(other_same_direction.float() * 2 - 1)
    ...
    def save_ply(self, path, mask=None,orient_normals=False):
    ...
    def load_ply(self, path):
        ...
        self._normals_mask = torch.zeros(torch.tensor(opacities).shape, dtype=torch.float, device="cuda")[:,0]
        self._features_dc = nn.Parameter(torch.tensor(features_dc, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(torch.tensor(features_extra, dtype=torch.float, device="cuda").transpose(1, 2).contiguous().requires_grad_(True))
        self._opacity = nn.Parameter(torch.tensor(np.copy(opacities), dtype=torch.float, device="cuda").requires_grad_(True))
