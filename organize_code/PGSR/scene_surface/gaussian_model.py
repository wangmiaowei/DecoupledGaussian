...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        #self.features_dc2,self.features_rest2,self.opacity2


        l = [
            {'params': [self.xyz2], 'lr': 0.00002, "name": "xyz"},
            #{'params': [self._knn_f], 'lr': 0.01, "name": "knn_f"},
            {'params': [self.features_dc2], 'lr': training_args.feature_lr, "name": "f_dc"},
            {'params': [self.features_rest2], 'lr': training_args.feature_lr / 20.0, "name": "f_rest"},
            {'params': [self.opacity2], 'lr': training_args.opacity_lr, "name": "opacity"},
            {'params': [self.scales2], 'lr': 0.05*training_args.scaling_lr, "name": "scaling"},
            {'params': [self.rots2], 'lr': training_args.rotation_lr, "name": "rotation"}
        ]
    ...
    def update_learning_rate(self, iteration):
        ...
        self._features_dc = torch.cat([self.features_dc_half,self.features_dc2],dim=0)
        self._features_rest = torch.cat([self.features_rest_half,self.features_rest2],dim=0)
        self._opacity = torch.cat([self.opacity_half,self.opacity2],dim=0)
        self._scaling = torch.cat([self.scaling_half,self.scales2],dim=0).cuda()
        self._xyz = torch.cat([self.xyz2_half,self.xyz2],dim=0).cuda()

        self._rotation = torch.cat([self.rotation_half,self.rots2],dim=0)
        #self._features_rest self.features_rest2
        #self._opacity    self.opacity2
        """
    ...
    def save_ply(self, path, mask=None):
        ...
        # Assign the points to the PointCloud
        point_cloud.points = o3d.utility.Vector3dVector(xyz)

        # Save the point cloud to a .ply file
        o3d.io.write_point_cloud("output_hahahah.ply", point_cloud)

        print('xyz.shape: ',xyz.shape)
        normals = np.zeros_like(xyz)
        f_dc = self._features_dc.detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
        f_rest = (torch.zeros(self._features_rest.shape)).detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    ...
    def load_ply(self, model_path,obj_name):
        scene_path = os.path.join(model_path, "dense_poisson_scripts", obj_name + "_results", "patch_segment.ply")
        extra_path = os.path.join("../GaussianSplattingLightning/results_here",
                                  obj_name,
                                  "left_raw_gaussian.ply")
        real_repair_path = os.path.join(model_path, "dense_poisson_scripts", obj_name + "_results", "patch_segment_real.ply")


        xyz,features_dc,features_extra,opacities,scales,rots = self.load_single_ply(scene_path)
        xyz1,features_dc1,features_extra1,opacities1,scales1,rots1 = self.load_single_ply(extra_path)
        xyz2,features_dc2,features_extra2,opacities2,scales2,rots2 = self.load_single_ply(real_repair_path)
        opacities2 = inverse_sigmoid(0.2*torch.ones((xyz2.shape[0], 1), dtype=torch.float))

        self.xyz2 = nn.Parameter(xyz2.cuda().requires_grad_(True))
        self.xyz2_half = torch.cat([xyz.requires_grad_(False).cuda(),
                                      xyz1.requires_grad_(False).cuda()],dim=0).contiguous()
        
        self.features_dc2 = nn.Parameter(features_dc2.cuda().requires_grad_(True))

        self.features_dc_half = torch.cat([features_dc.requires_grad_(False).cuda(),
                                      features_dc1.requires_grad_(False).cuda()],dim=0).contiguous()
        self.features_rest2 = nn.Parameter(features_extra2.cuda().requires_grad_(True))

        self.features_rest_half = torch.cat([features_extra.requires_grad_(False).cuda(),
                                      features_extra1.requires_grad_(False).cuda()],dim=0).contiguous()
        
        self.opacity2 = nn.Parameter(opacities2.cuda().requires_grad_(True))

        self.opacity_half = torch.cat([opacities.requires_grad_(False).cuda(),
                                      opacities1.requires_grad_(False).cuda()],dim=0).contiguous()
        
        self.scaling_half = torch.cat([scales.requires_grad_(False).cuda(),
                                      scales1.requires_grad_(False).cuda()],dim=0).contiguous()
        self.scales2 = nn.Parameter(scales2.requires_grad_(True).cuda())
        
        self.rotation_half = torch.cat([rots.requires_grad_(False).cuda(),
                                        rots1.requires_grad_(False).cuda()],dim=0).requires_grad_(False)
        self.rots2 = nn.Parameter(rots2.cuda().requires_grad_(True))

        self.active_sh_degree = self.max_sh_degree
        knn_f = torch.randn((xyz.shape[0], 6)).float().cuda()
        self._knn_f = nn.Parameter(knn_f.requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self.max_weight = torch.zeros((self.get_xyz.shape[0]), device="cuda")
