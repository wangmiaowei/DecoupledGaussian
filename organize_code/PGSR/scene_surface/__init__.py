...
from scene_surface.dataset_readers import sceneLoadTypeCallbacks
from scene_surface.gaussian_model import GaussianModel
from arguments import ModelParams
from utils.camera_utils_surface import cameraList_from_camInfos, camera_to_JSON
import numpy as np
import torch
...
class Scene:
    ...
    def __init__(self,obj_name, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if self.loaded_iter:
            self.gaussians.load_ply(self.model_path, self.obj_name)
    
    def save(self, iteration, mask=None):
        point_cloud_path = os.path.join(self.model_path, "point_cloud_scene/iteration_{}".format(iteration))
